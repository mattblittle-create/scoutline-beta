// app/admin/billing/payouts/page.tsx

import Link from "next/link";
import { redirect } from "next/navigation";
import { prisma } from "@/lib/prisma";
import { getAdminContext } from "@/lib/admin/getAdminContext";
import RecomputeCommissionsPanel from "./RecomputeCommissionsPanel";
...
export default async function AdminBillingPayoutsPage() {
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1200, fontFamily: "Arial, sans-serif", fontSize: 11 }}>
      ...
      >
        <div>
          ...
          <div style={{ opacity: 0.75, marginTop: 6 }}>
            Referral commissions + payout readiness. (Read-only V1 + recompute)
          </div>

          <div style={{ marginTop: 10, display: "flex", gap: 10, flexWrap: "wrap" }}>
            <a href="/admin/billing/payouts" style={a}>
              Refresh
            </a>
            <Link href="/admin/billing/invoices" style={a}>
              Invoices
            </Link>
      ...
      {/* ✅ Recompute panel (client) */}
      <section style={card}>
        <div style={{ fontWeight: 900, marginBottom: 8 }}>Recompute</div>
        <RecomputeCommissionsPanel />
        <div style={{ marginTop: 10, opacity: 0.75, lineHeight: 1.4 }}>
          Source of truth is invoices. $0 paid invoices don’t make a referral eligible until a &gt;$0 PAID invoice exists.
        </div>
      </section>
      ...
      <section style={card}>
        ...
        {bucketRows.length === 0 ? (
          <div style={{ opacity: 0.75 }}>No commission events yet.</div>
        ) : (
              ...
              <tbody>
                {bucketRows.map((r) => (
                  <tr key={r.payeeUserId}>
                    ...
                    <td style={td}>
                      <span style={{ fontWeight: 900 }}>{usdFromCents(r.eligibleCents)}</span>
                    </td>
                    <td style={td}>
                      <span style={{ fontWeight: 900 }}>{usdFromCents(r.pendingCents)}</span>
                    </td>
      ...
      <section style={card}>
        ...
        {commissions.length === 0 ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Earned", "Eligible At", "Status", "Referrer", "Target", "Plan", "Billed", "Commission", "Payout"].map(
                    (h) => (
                      <th key={h} style={th}>
                        {h}
                      </th>
                ...
                {commissions.slice(0, 250).map((c) => {
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td style={td}>{fmtDateTime((c as any).eligibleAt ?? null)}</td>
                      <td style={td}>
                        <code>{String(c.status)}</code>
                      </td>
                      ...
                      <td style={td}>
                        <code>{c.planTier}</code> / <code>{c.cadence}</code>
                      </td>
                      <td style={td}>{usdFromCents(c.billedAmountCents || 0)}</td>
                      <td style={td}>
                        <span style={{ fontWeight: 900 }}>{usdFromCents(c.commissionAmountCents || 0)}</span>
                      </td>
        ...
        <div style={{ marginTop: 8, opacity: 0.75 }}>Showing up to 250 most recent rows.</div>
      </section>
    </main>
  );
}
