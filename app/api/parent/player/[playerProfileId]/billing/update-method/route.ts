// app/api/parent/player/[playerProfileId]/billing/update-method/route.ts
...
export async function POST(_req: Request, ctx: Ctx) {
  try {
    ...
    return NextResponse.json({
      ...
      message:
        "Payment method update flow is not connected yet for this player account. The parent-side action is wired and ready for provider integration.",
    });
  } catch (err: any) {
    console.error("[parent update method] error", err);
    return NextResponse.json(
      { ok: false, error: err?.message || "Failed to open payment method flow." },
      { status: 500 }
    );
  }
}