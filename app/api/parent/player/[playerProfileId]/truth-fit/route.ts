// app/api/parent/player/[playerProfileId]/truth-fit/route.ts
...
async function getParentLinkedPlayerProfile({
  userId,
  playerProfileId,
}: {
  userId: string;
  ...
  const profile = parentProfile?.playerLinks?.[0]?.playerProfile || null;

  if (!profile) return null;

  const userPlayer = profile.user?.Player || null;
  ...
  const totalHeightIn =
    heightFt != null && heightInOnly != null
      ...
      : asNumber(userPlayer?.heightIn) ?? heightInOnly;
  ...
  return {
    ...
    player: {
      ...
      fullName:
        [asString(normalized?.firstName), asString(normalized?.lastName)]
          .filter(Boolean)
          .join(" ") || null,

      gpa: asNumber(userPlayer?.gpa) ?? asNumber(normalized?.gpa),
      gradYear:
        asNumber(userPlayer?.gradYear) ?? asNumber(normalized?.gradYear),
      primaryPos:
        asString(userPlayer?.primaryPos) ?? asString(normalized?.primaryPos),
      secondaryPos:
        asString(userPlayer?.secondaryPos) ??
        asString(normalized?.secondaryPos),
      isPitcher: asString(normalized?.isPitcher),
      pitcherHand:
        asString(userPlayer?.pitcherHand) ??
      ...
      weightLb:
        asNumber(userPlayer?.weightLb) ?? asNumber(normalized?.weightLb),
      metrics:
...
export async function GET(
  req: NextRequest,
  { params }: { params: { playerProfileId: string } }
) {
  try {
    ...
    const profile = await getParentLinkedPlayerProfile({
      userId: user.id,
      playerProfileId,
    });
...
return NextResponse.json({
  ...
  filters: {
    ...
    console.error("PARENT_TRUTH_FIT_ERROR", err);
    return NextResponse.json(
