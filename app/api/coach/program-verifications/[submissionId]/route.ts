// app/api/coach/program-verifications/[submissionId]/route.ts
...
function cleanNilStrength(value: unknown) {
  ...
  if (
    ["ELITE", "STRONG", "COMPETITIVE", "EMERGING", "LIMITED", "UNKNOWN"].includes(v)
  ) {
    return v as any;
  }
...
function buildProgramUpdate(data: any) {
  return {
    ...
    programXUrl: cleanString(data?.programXUrl, 1000),
    programInstagramUrl: cleanString(data?.programInstagramUrl, 1000),
    programYoutubeUrl: cleanString(data?.programYoutubeUrl, 1000),
...
export async function PATCH(
  ...
  if (submission.collegeId !== user.collegeId) {
    return NextResponse.json<Err>(
      { ok: false, error: "Unauthorized for this college." },
      { status: 403 }
    );
  }
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    await tx.college.update({
      ...
      data: {
        ...
        programInstagramUrl: programUpdate.programInstagramUrl,
        programXUrl: programUpdate.programXUrl,
