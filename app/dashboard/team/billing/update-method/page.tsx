// app/dashboard/team/billing/update-method/page.tsx
...
function TeamUpdateMethodContent() {
  ...
  async function saveToken(result: any) {
    ...
    const saveRes = await fetch(
      "/api/team/billing/payment-method-token/save",
      {
        method: "POST",
    ...
    setSuccess("Team payment information updated successfully.");
  }
    ...
    async function loadToken() {
      try {
        ...
        const res = await fetch(
          "/api/team/billing/payment-method-token/page-token",
          { method: "POST" }
        );
  ...
  useEffect(() => {
    ...
    try {
      ...
      passageRef.current = new window.PassageJS({
        clientToken,
        epi,
        formAction: "/api/team/billing/payment-method-token/save",
        formMethod: "POST",
        variant: "inline",
  ...
  }, [scriptReady, clientToken, epi, teamId]);
  ...
  return (
    ...
    >
      ...
      <h1 style={{ fontSize: 24, marginBottom: 8 }}>Update Team Payment Info</h1>

      <p style={{ opacity: 0.75, marginBottom: 24 }}>
        Your card will NOT be charged. This updates the payment method used for
        future team recurring billing.
      </p>
...
export default function TeamUpdateMethodPage() {
  return (
    <Suspense fallback={<div style={{ padding: 24 }}>Loading payment form...</div>}>
      <TeamUpdateMethodContent />
    </Suspense>
  );
}