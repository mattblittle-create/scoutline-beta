// scripts/enrich-baseball-coaches-dom.ts
...
type DivisionArgument =
  | SupportedDivision
  | "JUCO";

const SUPPORTED_DIVISIONS: readonly SupportedDivision[] = [
  "NCAA_D1",
  "NCAA_D2",
  "NCAA_D3",
  "NAIA",
...
function getEqualsOrNextArgument(flag: string): string | undefined {
  const equalsArg = process.argv.find((arg) =>
    arg.startsWith(`${flag}=`),
  );

  if (equalsArg) {
    return equalsArg.slice(`${flag}=`.length).trim() || undefined;
  }
...
function normalizeDivisionArgument(
  rawValue: string | undefined,
): DivisionArgument {
  const normalized = String(rawValue || "NCAA_D1")
    .trim()
    .toUpperCase()
    .replace(/[-\s]+/g, "_");

  const aliases: Record<string, DivisionArgument> = {
    D1: "NCAA_D1",
    NCAA1: "NCAA_D1",
    NCAA_D1: "NCAA_D1",
  ...
  if (!resolved) {
    throw new Error(
      `Unsupported --division value "${rawValue}". ` +
        `Use one of: ${SUPPORTED_DIVISIONS.join(", ")}, or JUCO.`,
    );
  }
...
const DIVISION_ARGUMENT = normalizeDivisionArgument(
  getEqualsOrNextArgument("--division"),
);

const DIVISION_FILTERS: SupportedDivision[] =
  DIVISION_ARGUMENT === "JUCO"
...
const DIVISION_FILE_KEY = DIVISION_LABEL;

/*
 * Stores the last successful URL pattern for each school.
 *
 * Each division gets its own cache so newly learned D2/D3/NAIA/NJCAA
 * patterns cannot change the proven D1 cache behavior.
 *
 * For NCAA_D1 only, the loader can fall back to the legacy D1 cache file
 * so the generalized scraper can regression-test against the current system.
 */
const SUCCESSFUL_PATTERN_CACHE_FILE = path.join(
  process.cwd(),
  "data",
  "enrichment",
  `college-baseball-coach-url-patterns.${DIVISION_FILE_KEY}.json`,
);

const LEGACY_D1_PATTERN_CACHE_FILE = path.join(
  process.cwd(),
  "data",
  "enrichment",
...
const OUT_FILE = path.join(
  OUT_DIR,
  `college-baseball-coaches.${DIVISION_FILE_KEY}.dom.generated.${stamp}.csv`,
);
...
function loadSuccessfulPatternCache(): SuccessfulPatternCache {
  try {
    let cacheFile = SUCCESSFUL_PATTERN_CACHE_FILE;

    if (
      !fs.existsSync(cacheFile) &&
      DIVISION_ARGUMENT === "NCAA_D1" &&
      fs.existsSync(LEGACY_D1_PATTERN_CACHE_FILE)
    ) {
      cacheFile = LEGACY_D1_PATTERN_CACHE_FILE;

      console.log(
        `Using legacy D1 URL-pattern cache: ${LEGACY_D1_PATTERN_CACHE_FILE}`,
      );
    }
...
function printRunSummary(stats: RunStats) {
  ...
  console.log("");
  console.log("=================================================");
  console.log(`${DIVISION_LABEL} BASEBALL COACH ENRICHMENT SUMMARY`);
  console.log("=================================================");
...
async function main() {
  ...
  console.log(
    `Division: ${DIVISION_LABEL} (${DIVISION_FILTERS.join(", ")})`,
  );
...
const stats: RunStats = {
  ...
  console.log(`Scanning ${programs.length} ${DIVISION_LABEL} programs...`);
