// scripts/set-liu-baseball-url.ts
...
const CORRECT_BASEBALL_URL =
  "https://www.liuathletics.com/sports/baseball";
...
async function main() {
  ...
  if (!APPLY) {
    ...
    console.log(
      "npx tsx scripts/set-liu-baseball-url.ts --apply",
    );
  ...
  if (
    ...
    throw new Error(
      "LIU URL update could not be verified.",
    );
  }

  console.log(
    "LIU baseball URL updated successfully.",
  );
}
