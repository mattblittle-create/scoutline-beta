// scripts/set-ccsu-baseball-url.ts
...
const CORRECT_BASEBALL_URL =
  "https://ccsubluedevils.com/sports/bsb";
...
async function main() {
  ...
  console.log(
    "SET CENTRAL CONNECTICUT STATE BASEBALL URL",
  );
  console.log(
    "=".repeat(80),
  );
  ...
  if (!APPLY) {
    ...
    console.log(
      "npx tsx scripts/set-ccsu-baseball-url.ts --apply",
    );
  ...
  if (
    ...
    throw new Error(
      "Central Connecticut State URL update could not be verified.",
    );
  }

  console.log(
    "Central Connecticut State baseball URL updated successfully.",
  );
}
